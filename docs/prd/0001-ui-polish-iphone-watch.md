...
- **Anything gated behind a paid Apple account** — Activity ring credit, and the constraints that come with the free tier, are accepted as given. *HealthKit was named first here for a long time, and that was wrong: it signs on a free personal team, and the phone now records finished workouts to Apple Health. Ring credit is still out of reach without an `HKWorkoutSession` — see [`docs/decisions.md`](../decisions.md).* *This non-goal is what the Lock Screen surface ran into, and it is exactly the case the note below anticipated: the surface went, and the non-goal stood. It went for a free-tier reason, though not the one question 10 asked about — see the second amendment.*
